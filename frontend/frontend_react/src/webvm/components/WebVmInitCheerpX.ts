...
import { blockCacheAtom, cxReadFuncAtom } from '../WebVmAtoms'
...
function readData(
  str: string,
  readFunc: ((char: number) => void) | null,
): void {
  if (!readFunc) {
...
export async function initCheerpX(term: Terminal | null): Promise<void> {
  ...
  try {
    ...
    const blockDevice = await CheerpX.GitHubDevice.create(
    ...
    term.scrollToBottom()
    const readFunc = cheerpX.setCustomConsole(
      (buf: Uint8Array, vt: number) => writeData(term, buf, vt),
      term.cols,
      term.rows,
    )
    console.log(
      'CheerpX console connected, readFunc:',
      readFunc,
      'cols:',
      term.cols,
      'rows:',
